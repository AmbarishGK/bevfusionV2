...
parser.add_argument("--resume_from", default=None, help="Path to a specific checkpoint dir (overrides --resume)")

# Perf / precision
parser.add_argument("--precision", choices=["bf16", "fp16"], default="bf16", help="Autocast dtype")
parser.add_argument("--attn_impl", choices=["auto", "sdpa", "flash_attention_2"], default="auto")
parser.add_argument("--tf32", action="store_true", help="Enable TF32 matmul (recommended on A100/H100)")
parser.add_argument("--torch_compile", action="store_true", help="Use torch.compile for faster training (PyTorch 2.x)")
parser.add_argument("--grad_ckpt", action="store_true", help="Enable gradient checkpointing (saves VRAM, slower)")
...
parser.add_argument("--save_total_limit", type=int, default=5, help="Keep last N checkpoints (0=keep all)")
...
if checkpoints:
    ...
    if (not args.resume) and (not args.resume_from):
        warnings.append("Checkpoints exist but --resume/--resume_from not set. Will restart from scratch.")
    else:
...
if attn_impl == "auto":
    # Prefer FlashAttention2 when available (best on H100), else SDPA.
...
try:
    ...
    if attn_impl == "flash_attention_2":
        print(f"  ⚠️  flash_attention_2 init failed ({e}). Falling back to SDPA.")
        attn_impl = "sdpa"
        base_model = transformers.Qwen3VLForConditionalGeneration.from_pretrained(
            args.model,
            quantization_config=bnb_config,
...
if args.torch_compile:
    try:
        ...
        warnings.append(f"torch.compile failed; continuing without compile: {e}")
...
# Resume optimizer/scheduler/global_step if trainer_state.pt is present
start_epoch = 0
if resume_ckpt and resume_ckpt.exists():
    ...
    if st:
        try:
            ...
            print(f"  ✅ Resumed trainer state: epoch={start_epoch} global_step={global_step} best_val_loss={best_val_loss:.4f}")
        except Exception as e:
            print(f"  ⚠️  Could not restore full trainer state; continuing partially: {e}")
            ...
            if (step + 1) % args.grad_accum == 0:
                ...
                if global_step % args.log_steps == 0 or args.dry_run:
                    ...
                    print(f"  step {global_step:>5} | loss {epoch_loss/(step+1):.4f} "
                          ...
                          f"| tokens/s {tok_s:,.0f} | fused_opt {fused_opt} | attn {attn_impl} | prec {args.precision}")
                    step_t0 = time.time()
                    tokens_seen = 0

                # Save checkpoint
                if (not args.dry_run) and (args.save_steps > 0) and (global_step % args.save_steps == 0):
                    ...
                    print(f"  💾 Saved checkpoint-{global_step} (adapter + trainer_state.pt)")
