...
def save_bev_map(points, detections, out_path, scene_name="", source="GT",
    ...
    # ── Range rings ──────────────────────────────────────────
    # Solid inner rings (10m, 20m) are most useful for driving decisions
    for r, lw, ls, col, label_col in [
        (10, 1.2, '-',  '#2a3a2a', '#4a7a4a'),   # solid, green tint — danger zone
        (20, 1.0, '-',  '#1e2a1e', '#3a5a3a'),
        (30, 0.7, '--', '#1a2020', '#2a4040'),
        (40, 0.6, '--', '#161a1a', '#243030'),
        (50, 0.5, ':', '#121616', '#1a2424'),
    ]:
        ax.add_patch(plt.Circle((0, 0), r, color=col, fill=False,
                                linewidth=lw, linestyle=ls, zorder=1))
        # Label at top of each ring so it doesn't overlap point cloud
        ax.text(0, r + 0.8, f'{r}m', color=label_col,
                fontsize=8.5, fontweight='bold', zorder=1,
                ha='center', va='bottom')

    # ── Forward zone indicator (±22.5° = directly ahead cone) ─
    import matplotlib.patches as mpatches
    wedge = mpatches.Wedge((0, 0), BEV_RANGE_M,
                            theta1=67.5, theta2=112.5,
                            color='#0d1f0d', alpha=0.12, zorder=1)  # very subtle
    ax.add_patch(wedge)
    ax.text(0, BEV_RANGE_M * 0.88, '▲ FORWARD', color='#3a7a3a',
            fontsize=8.0, ha='center', va='center', zorder=2,
    ...
    if points is not None and len(points) > 0:
        ...
        pts_y   = points[:, 1]
        pts_z   = points[:, 2]
        dist2d  = np.sqrt(pts_x**2 + pts_y**2)

        # Tighter ground filter: remove road surface AND very close ego-shadow
        # z > -0.8 keeps vehicle bodies, curbs, low objects; removes flat road
        # dist > 2.5 removes the immediate ego shadow which blows out near-ego
        mask = (pts_z > -0.8) & (dist2d >= 2.5) & (dist2d <= 45.0)
        pts_x, pts_y, pts_z, dist2d = (pts_x[mask], pts_y[mask],
                                        pts_z[mask], dist2d[mask])

        if len(pts_x) > 0:
            # Clamp z so colour is driven by object height not outliers
            z_lo = np.percentile(pts_z, 8)
            z_hi = np.percentile(pts_z, 92)

            # 8 distance bins — near large+bright, far still visible
            # Key change: flatten the alpha curve so outer bins are legible
            bin_edges = np.linspace(0, 45, 9)
            for i in range(len(bin_edges) - 1):
                d_lo, d_hi = bin_edges[i], bin_edges[i+1]
                in_bin = (dist2d >= d_lo) & (dist2d < d_hi)
                if not np.any(in_bin):
                    continue
                prox     = 1.0 - (d_lo / 45.0)          # 1.0 near → 0.0 far
                # Size: 1.2 (far) → 3.8 (near) — reduced near to cut blowout
                pt_size  = 1.2 + prox ** 2.2 * 2.6
                # Alpha: 0.28 (far) → 0.62 (near) — raised floor for outer visibility
                pt_alpha = 0.28 + prox ** 1.5 * 0.34
                ax.scatter(px(pts_x[in_bin], pts_y[in_bin]),
                           py(pts_x[in_bin], pts_y[in_bin]),
                           s=pt_size, c=pts_z[in_bin], cmap='YlOrRd',
                           vmin=z_lo, vmax=z_hi,
                           alpha=pt_alpha, linewidths=0, zorder=3)
    ...
    if radar_pts is not None and radar_pts.shape[0] > 0:
        ...
        if r_vis.shape[0] > 0:
            # Outer white ring + cyan fill makes radar unmistakable vs LiDAR
            ax.scatter(px(r_vis[:,0], r_vis[:,1]),
                       py(r_vis[:,0], r_vis[:,1]),
                       s=55, c='#00d4ff', alpha=0.9,
                       linewidths=1.2, edgecolors='white', zorder=5)

            # Velocity arrows — only for matched detections (no ghost arrows)
    ...
    ax.set_xlabel('← left  |  right →  (metres)',   color='#6e7681', fontsize=10)
    ax.set_ylabel('← back  |  forward →  (metres)', color='#6e7681', fontsize=10)
    ax.set_title(f"MoRAL BEV [{source}] — {scene_name}",
                 color='#e6edf3', fontsize=13, fontweight='bold', pad=15)
    ax.tick_params(colors='#4a5260', labelsize=8)
    for sp in ax.spines.values():
        sp.set_color('#21262d')
    ax.set_aspect('equal')
    # Subtle grid at axis labels only — no internal grid lines
