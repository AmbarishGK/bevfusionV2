...
# ─────────────────────────────────────────────────────────────
#  Scene text generation — rich format for Cosmos QA generation
# ─────────────────────────────────────────────────────────────

# Ego vehicle dimensions (nuScenes data collection vehicle)
EGO_WIDTH_M  = 2.0
EGO_LENGTH_M = 4.5

# nuScenes typical braking deceleration (m/s²)
BRAKE_DECEL  = 4.0
...
def _bearing_to_zone(bearing_deg):
    b = bearing_deg
...
def _best_velocity(d, has_radar):
    """Return (speed_ms, source_label) — prefer radar Doppler over GT estimate."""
    if has_radar and d.get('radar_velocity_confirmed'):
        rv = d.get('radar_velocity_ms')
        if rv is not None:
            return rv, 'radar'
    return d.get('velocity_ms', 0.0), 'gt'

def _stopping_distance(speed_ms, decel=BRAKE_DECEL):
    """v² / 2a"""
    return round(speed_ms**2 / (2 * decel), 1)

def _time_to_collision(distance_m, obj_speed, ego_speed, bearing_deg):
    """
    Rough TTC assuming object is moving toward ego if it is ahead.
    Positive = object approaching ego's path.
    """
    # Only compute for objects roughly in front or behind
    if abs(bearing_deg) < 45 or abs(bearing_deg) > 135:
        closing = ego_speed + obj_speed  # worst case
        if closing > 0.1:
...
def _gap_passable(d):
    """
    Check if the ego vehicle can pass beside this object
    given the object's width and typical lane width (3.7m).
    """
    obj_w = d.get('width_m', 2.0)
    gap   = 3.7 - obj_w / 2 - EGO_WIDTH_M / 2
    return gap, gap >= 0.3  # 0.3m minimum clearance

def make_scene_text(detections, ego_speed=None, include_velocity=True,
                    max_objects=None):
    """
    Generate a rich, structured natural language scene description
    optimised for Cosmos-Reason2 QA pair generation.

    The description is designed to enable Cosmos to ask and answer:
      - Spatial questions  ("What is directly ahead?")
      - Distance questions ("How far is the nearest pedestrian?")
      - Physics questions  ("What is the stopping distance at current speed?")
      - Safety questions   ("Which objects pose an immediate collision risk?")
      - Occlusion questions("Which objects are hidden from camera view?")
      - Gap questions      ("Is there room to pass the truck?")
      - Comparative questions ("Which car is faster?")

    max_objects=None means include ALL detections (no truncation).
    For Cosmos QA generation we want the full scene.
    """
    if not detections:
    ...
    es        = ego_speed or 0.0
    dets      = detections if max_objects is None else detections[:max_objects]

    lines = []

    # ── 1. Scene header ──────────────────────────────────────
    n_total   = len(detections)
    n_moving  = sum(1 for d in detections if _best_velocity(d, has_radar)[0] > 0.5)
    n_occluded= sum(1 for d in detections
                    if d.get('visibility') in ['mostly_occluded', 'partially_visible'])
    data_src  = "LiDAR+camera BEVFusion fusion" if is_bev else "LiDAR ground-truth annotations"
    radar_note= " Radar Doppler velocities are physically measured." if has_radar else ""

    lines.append(
        f"SCENE OVERVIEW: {n_total} objects detected via {data_src}."
        f" {n_moving} are moving. {n_occluded} are partially or fully occluded."
        f"{radar_note}"
    )

    # ── 2. Ego vehicle state ─────────────────────────────────
    if ego_speed is not None:
        stop_d = _stopping_distance(es)
        lines.append(
            f"EGO STATE: Travelling at {es:.1f} m/s ({es*3.6:.1f} km/h)."
            f" Estimated stopping distance at current speed: {stop_d} m."
            f" Ego vehicle dimensions: {EGO_LENGTH_M} m long, {EGO_WIDTH_M} m wide."
        )

    # ── 3. Nearest object per zone ───────────────────────────
    zones = {}
    for d in detections:
        z = _bearing_to_zone(d['bearing_deg'])
    ...
    for z in zone_order:
        if z in zones:
            d    = zones[z]
            spd, src = _best_velocity(d, has_radar)
            vel_str  = f", moving at {spd:.1f} m/s ({'radar-confirmed' if src=='radar' else 'estimated'})" \
                       if spd > 0.3 else ""
            zone_summary.append(
                f"{z}: {d['class']} at {d['distance_m']} m{vel_str}"
            )
    if zone_summary:
        lines.append("NEAREST OBJECT PER ZONE: " + "; ".join(zone_summary) + ".")

    # ── 4. Safety-critical objects ───────────────────────────
    critical = []
    for d in dets:
        spd, src = _best_velocity(d, has_radar)
        ttc = _time_to_collision(d['distance_m'], spd, es, d['bearing_deg'])
    ...
    if critical:
        crit_lines = []
        for ttc_val, d, spd, src in critical[:5]:
            vel_str = f" at {spd:.1f} m/s ({'radar' if src=='radar' else 'estimated'})" \
                      if spd > 0.3 else ""
            crit_lines.append(
                f"{d['class']} {_bearing_to_zone(d['bearing_deg'])}"
                f" {d['distance_m']} m away{vel_str}"
                f" (TTC ≈ {ttc_val:.1f} s)"
            )
        lines.append("SAFETY CRITICAL: " + "; ".join(crit_lines) + ".")

    # ── 5. Full object list — all detections ─────────────────
    lines.append("FULL OBJECT LIST:")
    for i, d in enumerate(dets):
        zone        = _bearing_to_zone(d['bearing_deg'])
        spd, src    = _best_velocity(d, has_radar)
        vis         = d.get('visibility', 'unknown')
        w, l, h     = d.get('width_m', 0), d.get('length_m', 0), d.get('height_m', 0)
        lidar_pts   = d.get('num_lidar_pts', -1)
        rcs         = d.get('radar_rcs')

        parts = [
            f"[{i+1}] {d['class']}",
            f"zone={zone}",
            f"distance={d['distance_m']}m",
            f"bearing={d['bearing_deg']}deg",
            f"size={w:.1f}x{l:.1f}x{h:.1f}m (WxLxH)",
        ]

        # Velocity
        if spd > 0.3:
            vel_label = 'radar-confirmed' if src == 'radar' else 'gt-estimated'
            parts.append(f"speed={spd:.2f}m/s ({vel_label})")
        else:
            parts.append("speed=stationary")

        # Confidence (BEVFusion only)
        if is_bev:
            parts.append(f"confidence={int(d.get('confidence',1.0)*100)}%")

        # Visibility / occlusion
        parts.append(f"visibility={vis}")

        # LiDAR evidence strength
        if lidar_pts >= 0:
            pts_label = "strong" if lidar_pts > 50 else ("moderate" if lidar_pts > 10 else "weak")
            parts.append(f"lidar_pts={lidar_pts}({pts_label})")

        # Radar RCS (object size proxy)
        if rcs is not None:
        ...
        # Passability check for wide objects
        if d['class'] in ['truck', 'bus', 'construction_vehicle', 'car']:
            gap, passable = _gap_passable(d)
            parts.append(f"lane_gap≈{gap:.1f}m ({'passable' if passable else 'tight'})")

        lines.append("  " + ", ".join(parts) + ".")

    # ── 6. Object type counts ────────────────────────────────
    counts = {}
    for d in detections:
        counts[d['class']] = counts.get(d['class'], 0) + 1
    count_str = ", ".join(f"{v} {k}{'s' if v>1 else ''}" for k, v in sorted(counts.items()))
    lines.append(f"OBJECT COUNTS: {count_str}.")

    # ── 7. Occlusion summary ─────────────────────────────────
    occluded = [d for d in detections
                if d.get('visibility') in ['mostly_occluded', 'partially_visible']]
    if occluded:
        occ_parts = []
        for d in occluded[:8]:
            occ_parts.append(
                f"{d['class']} at {d['distance_m']}m "
                f"{_bearing_to_zone(d['bearing_deg'])} ({d.get('visibility','')})"
            )
        lines.append(
